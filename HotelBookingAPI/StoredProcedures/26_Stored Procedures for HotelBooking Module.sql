-- This SQL script contains the stored procedures for the HotelBooking module
-- It includes procedures for searching available rooms, calculating room costs, creating reservations, adding guests to reservations, and processing payments
...
-- This SQL command drops the foreign key constraint related to RoomID in the Reservations table
ALTER TABLE Reservations

DROP CONSTRAINT FK__Reservati__RoomI__6383C8BA;

-- This SQL command drops the RoomID and NumberOfGuests columns from the Reservations table
ALTER TABLE Reservations

DROP COLUMN RoomID
	,NumberOfGuests;

-- This SQL command adds the TotalCost and NumberOfNights columns to the Reservations table
ALTER TABLE Reservations ADD TotalCost DECIMAL(10, 2)
	,NumberOfNights INT;

-- This SQL command creates a new table for the Reservation-Rooms linkage
...
-- First, delete the ReservationGuests table if it already exists:
...
CREATE TABLE ReservationGuests (
	...
	,-- Linking directly to the ReservationRooms table
	GuestID INT
...
CREATE TABLE Payments (
	...
	,PaymentStatus NVARCHAR(50) DEFAULT 'Pending' CHECK (
		PaymentStatus IN (
			'Pending'
			,'Completed'
			,'Failed'
...
CREATE TABLE PaymentDetails (
	...
	NumberOfNights INT
	,GST DECIMAL(10, 2)
	,-- GST Based on the Base Amount
	TotalAmount DECIMAL(10, 2)
	,-- (Amount * NumberOfNights) + GST
...
ALTER PROCEDURE spSearchByAvailability @CheckInDate DATE
	,@CheckOutDate DATE
AS
BEGIN
...
ALTER PROCEDURE spCalculateRoomCosts @RoomIDs RoomIDTableType READONLY
	,@CheckInDate DATE
	,@CheckOutDate DATE
	,@Amount DECIMAL(10, 2) OUTPUT
	,-- Base total cost before tax
	@GST DECIMAL(10, 2) OUTPUT
	,-- GST amount based on 18%
...
ALTER PROCEDURE spCreateReservation @UserID INT
	,@RoomIDs RoomIDTableType READONLY
	,-- Using the table-valued parameter
	@CheckInDate DATE
	,@CheckOutDate DATE
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @BaseCost = SUM(Price * @NumberOfNights)
		FROM Rooms
		WHERE RoomID IN (
				SELECT RoomID
				FROM @RoomIDs
...
ALTER PROCEDURE spAddGuestsToReservation @UserID INT
	,@ReservationID INT
...
ALTER PROCEDURE spProcessPayment @ReservationID INT
	,@TotalAmount DECIMAL(10, 2)
	,@PaymentMethod NVARCHAR(50)
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @PaymentID
			...
			,r.Price
			,@NumberOfNights
			,(r.Price * @NumberOfNights) * 0.18
			,r.Price * @NumberOfNights + (r.Price * @NumberOfNights) * 0.18
		FROM ReservationRooms rr
